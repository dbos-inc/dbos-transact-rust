...
-- application_name, written to the column migration 100 added. The parameter is
-- last and defaults to NULL, so a caller that omits it — an older SDK, or a
-- client that has no application name to claim — enqueues an unclaimed workflow
-- exactly as before. The old 16-arg function is dropped first so only the new
-- signature remains; leaving both would make a 16-argument call ambiguous.
