...
//! reads back what the first execution did instead of doing it again. That matters most for
//! [`fork`](DBOS::fork): the fork's id is generated by the call, and without a checkpoint a
//! replayed workflow would fork a second time under a second id. It matters for the rest too —
//! a listing replays the rows it saw, and a cancel or a delete is issued once.
//!
//! The recorded names are the cross-SDK spellings — `DBOS.cancelWorkflow`, `DBOS.forkWorkflow`,
//! `DBOS.listWorkflows` — which is what Python's `call_function_as_step`, TypeScript's
//! `runInternalStep` and Go's `RunAsStep` write, so a step listing reads the same whichever
//! implementation recorded it. One call is one step, and the singular forms delegate to the bulk
//! ones, so `cancel` and `cancel_all` cost the same one step id.
//!
...
//! step's own checkpoint stands for everything its body did. And a **failure records nothing**, so
//! a replay makes the call again — what fails here is the database being unreachable rather than
...
impl DBOS {
    ...
    pub async fn cancel_all(
        ...
        let cancelled = internal_step("DBOS.cancelWorkflow", || async {
            executor
                .sysdb()
    ...
    pub async fn resume_all<R, E>(
        ...
        internal_step("DBOS.resumeWorkflow", || async {
            executor
                .sysdb()
    ...
    ///
    /// Called from inside a workflow, the fork is checkpointed as a step, so a replay hands back
    /// the id the first execution generated instead of writing a second fork.
    ///
    /// ```no_run
    /// # async fn f(dbos: &dbos::DBOS) -> dbos::Result<()> {
    ...
    pub async fn delete_all(&self, workflow_ids: &[&str], children: Children) -> Result<u64> {
        ...
        let deleted = internal_step("DBOS.deleteWorkflow", || async {
            executor
                .sysdb()
    ...
    pub async fn set_workflow_delay(&self, workflow_id: &str, delay: WorkflowDelay) -> Result<()> {
        ...
        internal_step("DBOS.setWorkflowDelay", || async {
            executor
                .sysdb()
    ...
    pub async fn update_workflow_attributes(
        ...
        internal_step("DBOS.updateWorkflowAttributes", || async {
            executor
                .sysdb()
    ...
    pub async fn list_workflows(&self, filter: &WorkflowFilter<'_>) -> Result<Vec<WorkflowRecord>> {
        ...
        internal_step("DBOS.listWorkflows", || async {
            executor
                .sysdb()
    ...
    pub async fn list_workflow_steps(&self, workflow_id: &str) -> Result<Vec<StepRecord>> {
        ...
        internal_step("DBOS.listWorkflowSteps", || async {
            executor
                .sysdb()
...
async fn fork_batch(
    ...
    internal_step("DBOS.forkWorkflow", || async {
        match from {
