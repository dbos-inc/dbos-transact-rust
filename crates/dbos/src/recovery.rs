...
use crate::sysdb::types::Submission;
...
/// Reads one abandoned workflow's row and submits it.
///
/// The fetch is all that is left here: everything after it — the registry lookup, the claim, the
/// spawn — is [`dispatch`](crate::dispatch::dispatch), which the queue runner reaches by the same
/// door with [`Submission::Dequeue`]. Recovery reads one row at a time because it holds one id at
/// a time; a dequeue reads its whole claimed batch at once, which is why the read stayed with the
/// callers rather than moving into the shared path.
