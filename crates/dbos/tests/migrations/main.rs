...
//! test binaries one after another and only parallelises *within* one, so two files would be two
//! thread pools used in sequence — each draining to a tail where most threads sit idle waiting
//! for the last few tests. Merged, twenty-three tests share one pool and short tests backfill
...
//! twelve cores, `runner`'s tests take ~40s together and blow an 800s timeout at
