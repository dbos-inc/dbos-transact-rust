...
//! on a branch's index, and a declarative macro can do neither — so it has to hand the work to a
//! function written per arity, with a sum type per arity to carry the winner's value back out.
//! That caps the fan-out at however many arities somebody wrote, and puts the shape of the
//! expansion in two places.
//!
//! A procedural macro is an ordinary Rust program over tokens, so it writes `__branch0`,
...
struct Arm {
    ...
    binding: Pat,
    /// The step. One expression, evaluated once, and evaluated before any branch is polled.
    step: Expr,
...
impl Parse for Race {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        while !input.is_empty() {
            ...
            // The `match` rule exactly: a block-shaped body ends itself, so the comma after it is
            // optional. Worth the dozen lines because the alternative is a macro that reads like
            // `match` until the moment somebody writes a block, and then does not.
        ...
        if arms.len() < 2 {
            ...
            // rather than at the macro name.
            let at = arms
                .first()
                .map_or_else(Span::call_site, |arm| arm.step.span());
            return Err(Error::new(
                at,
                "select_step! races two or more steps. One branch is not a race — await the step \
                 directly, which is both shorter and durable on its own.",
            ));
        }
...
impl Parse for Arm {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        if input.peek(Token![=>]) {
            return Err(Error::new(
                input.span(),
                "each arm of select_step! binds its step's outcome: `binding = step => \
...
/// Both spellings of a guard: tokio's `= step, if cond =>` and `match`'s `= step if cond =>`.
...
const GUARD: &str = "select_step! takes no guards. A branch that is present on the first run and \
                     ...
                     of a condition. Decide before the race and pick the step, or race a step \
                     that returns early.";
...
fn expand(race: &Race) -> TokenStream2 {
    ...
    quote! {{
        // **Every branch is built before any is polled.** A step takes its id when it is built,
        // so this is what fixes the ids: they follow source order, which is the order a replay
        // will build them in again.
        #( let mut #branch = #step; )*

        // Read while the branches are alive, because the losers are dropped as soon as the race
        // is decided and a stale-winner report has to be able to name a branch that is gone.
        // `push` is also what ties the branches to one error type: they differ in what they
        // return and they agree on how they fail.
        ...
        // procedural macro buys: it can write `#count` distinct names.
        ...
        let #winner = match ::dbos::__private::check_select(&#branches).await {
            ...
            ::core::result::Result::Ok(::dbos::__private::Racing::Replay(#at)) => {
                match #at {
                    ...
                    // `check_select` checks a recorded winner against the branch count before it
            ...
            ::core::result::Result::Ok(::dbos::__private::Racing::Fresh(#recording)) => {
                ...
                let #at = ::core::future::poll_fn(|#cx| {
                    #(
                        // Returned from inside, so nothing is polled after one went `Ready`.
                        // Fixed source order, never tokio's randomised one: fairness is exactly
                        // the property a replay cannot reproduce, so two branches ready in the
                        // same instant resolve to the earlier — and a replay reads the winner
        ...
        // One `match` over the index, rather than the arm bodies appearing once per path: a
        // replay and a fresh race reach the same place with the same slot filled.
        match #winner {
            ::core::result::Result::Err(#failed) => ::core::result::Result::Err(#failed),
            #(
