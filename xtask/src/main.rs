...
//! reason spelled out on [`execute_release`].
//!
...
fn run(args: &[String]) -> Result<()> {
    ...
    let branch = check_branch(&root, level)?;
    ...
    if dry_run {
        dry_run_release(&root, level, &branch, &released)
    } else {
...
fn execute_release(root: &Path, level: Level, released: &str) -> Result<()> {
    cargo_release(root, &["version", level.as_str()], Mode::Execute)?;
    cargo_release(root, &["commit"], Mode::Execute)?;
    // Only knowable once the bump is on disk, and worth knowing before the publish: cargo-release
    // tags after publishing, so a name already taken would otherwise fail an irreversible release.
    ensure_tag_free(root, &tag_name(root)?)?;
    cargo_release(root, &["publish"], Mode::Execute)?;
    cargo_release(root, &["tag"], Mode::Execute)?;
    cargo_release(root, &["push"], Mode::Execute)?;

    // Only a final release from main opens a new minor line and retargets main.
    if level != Level::Release {
        return Ok(());
    }
    post_release(root, released, Mode::Execute)
}

/// A dry run of the same thing, which has to bump, commit and tag for real to be worth anything.
///
/// cargo-release's own dry run leaves the manifests alone, so on this workspace it reports the
/// unsatisfiable `-dev` pin described on [`execute_release`] and packages crates still carrying
/// the `-dev` version — it never sees the artifacts a release would actually upload. The tag is
/// cut for a similar reason: `cargo release push` refuses to run without it, and a tag name
/// already taken is exactly what a dry run exists to find. Only the two steps that leave this
/// machine, the publish and the push, are simulated.
///
/// All of it happens on a detached HEAD, so the release branch itself never moves. An interrupted
/// dry run therefore cannot leave a release commit sitting on a branch for someone to push by
/// accident; the most it leaves behind is a detached HEAD and a local tag, both named in the
/// message below. On every other path out, including an error or a panic, [`Scratch`] undoes them.
fn dry_run_release(root: &Path, level: Level, branch: &str, released: &str) -> Result<()> {
    println!(
        "Dry run: bumping the version in a scratch commit on a detached HEAD, so the publish check \
         and the packaged crates are the real ones. {branch} does not move and nothing is pushed; \
         the commit, the tag and the detached HEAD are all undone before this returns.\n\
         If this is interrupted, `git checkout {branch}` gets you back, and `git tag` shows any \
         v-tag left to delete."
    );
    let mut scratch = Scratch::detach(root, branch)?;

    cargo_release(root, &["version", level.as_str()], Mode::Scratch)?;
    cargo_release(root, &["commit"], Mode::Scratch)?;

    // Recorded before the tag is cut, so a tag step that fails halfway is still cleaned up — and
    // after `ensure_tag_free`, so cleanup can never delete a tag that was already there.
    let tag = tag_name(root)?;
    ensure_tag_free(root, &tag)?;
    scratch.tag = Some(tag);

    cargo_release(root, &["publish"], Mode::DryRun)?;
    cargo_release(root, &["tag"], Mode::Scratch)?;
    cargo_release(root, &["push"], Mode::DryRun)?;

    if level != Level::Release {
        return Ok(());
    }
    // Still on the scratch commit, so this sees the release commit and the release-numbered
    // manifests a real post-release would, rather than the `-dev` ones the dry run started from.
    post_release(root, released, Mode::DryRun)
}

/// A dry run's detached HEAD and everything it leaves behind, undone when this is dropped.
///
/// Cleanup lives in `Drop` rather than after the steps so that it happens on every path out of
/// [`dry_run_release`] — the ordinary one, an early `?`, or a panic — and so that it runs *after*
/// the error it is unwinding: a cleanup that fails reports itself instead of replacing the failure
/// that mattered.
struct Scratch<'a> {
    root: &'a Path,
    /// The branch to return to. It never moved; only HEAD did.
    branch: String,
    head: String,
    /// Set once a tag is about to be cut, and only when that name was free to begin with.
    tag: Option<String>,
}

impl<'a> Scratch<'a> {
    fn detach(root: &'a Path, branch: &str) -> Result<Self> {
        let head = capture(root, "git", &["rev-parse", "HEAD"])?;
        sh(root, "git", &["checkout", "--quiet", "--detach", &head])?;
        Ok(Scratch {
            root,
...
impl Drop for Scratch<'_> {
    fn drop(&mut self) {
        if let Some(tag) = &self.tag {
            // `unwrap_or(true)`: if the check itself failed, attempt the delete and let git explain.
            let exists = ref_exists(self.root, &format!("refs/tags/{tag}")).unwrap_or(true);
            if exists && sh(self.root, "git", &["tag", "--delete", tag]).is_err() {
                eprintln!("warning: leftover dry-run tag {tag}; remove it with: git tag -d {tag}");
            }
        }
        let restored = sh(
            self.root,
            "git",
            &["reset", "--hard", "--quiet", &self.head],
        )
        .and_then(|()| sh(self.root, "git", &["checkout", "--quiet", &self.branch]));
        if restored.is_err() {
            eprintln!(
                "warning: could not return to {branch}; run: git checkout --force {branch} && \
                 git reset --hard {head}",
                branch = self.branch,
                head = self.head,
            );
...
fn post_release(root: &Path, released: &str, mode: Mode) -> Result<()> {
    ...
    if ref_exists(root, &format!("refs/heads/{branch}"))? {
        ...
        git(root, &["branch", &branch], mode)?;
        git(root, &["push", "origin", &branch], mode)?;
    }

    let next = format!("{major}.{}.0-dev", minor + 1);
    println!("Moving main from {released} to {next}");
    cargo_release(root, &["version", &next], mode)?;
    cargo_release(root, &["commit"], mode)?;
    git(root, &["push", "origin", "main"], mode)
}
...
fn check_branch(root: &Path, level: Level) -> Result<String> {
    let branch = capture(root, "git", &["rev-parse", "--abbrev-ref", "HEAD"])?;
    match level {
        Level::Rc | Level::Release if branch == "main" => Ok(branch),
        Level::Patch if branch.starts_with("release/v") => Ok(branch),
...
/// The tag `cargo release tag` will cut. Only knowable from the bumped manifests: for an `rc` or
/// a `patch`, cargo-release picks the number itself.
fn tag_name(root: &Path) -> Result<String> {
    Ok(format!("v{}", current_version(root)?))
}

fn ensure_tag_free(root: &Path, tag: &str) -> Result<()> {
    if ref_exists(root, &format!("refs/tags/{tag}"))? {
        return Err(format!(
            "the tag {tag} already exists; delete it (git tag -d {tag}) or release another version"
        )
        .into());
    }
    Ok(())
...
fn ref_exists(root: &Path, refname: &str) -> Result<bool> {
...
/// Whether a step does the thing or only says what it would do, and where it does it.
#[derive(Clone, Copy, PartialEq, Eq)]
enum Mode {
    /// Do it, on the branch the release is being cut from.
    Execute,
    /// Do it, on a dry run's scratch commit, to be undone before the dry run returns.
    Scratch,
...
impl Mode {
    ...
    /// Both dry-run modes work on the detached HEAD [`Scratch`] leaves them on.
...
fn cargo_release(root: &Path, args: &[&str], mode: Mode) -> Result<()> {
    ...
    if mode.detached() {
        // On a detached HEAD cargo-release sees the branch as `HEAD`, which `allow-branch` in
        // release.toml rejects. The branch was already checked, by rules allow-branch cannot state.
        argv.extend_from_slice(&["--allow-branch", "HEAD"]);
    }
    sh(root, "cargo", &argv)
}

/// git has no dry run, so a dry run describes the command instead of running it.
fn git(root: &Path, args: &[&str], mode: Mode) -> Result<()> {
    if mode.executes() {
